...
def test_input_sanitization() -> None:
    """Ensure malicious scripts are blocked."""
    from app import DemocracyQuestApp
    app = DemocracyQuestApp()
    assert "<script>" not in app.sanitize_input("<script>alert('1')</script>")

def test_database_write(mock_db_connection) -> None:
    """Ensure parameters save to DB properly."""
...
def test_db_failure(mock_connect) -> None:
    """Ensure application doesn't crash on DB failure."""
    from app import DemocracyQuestApp
...
def test_missing_api_key(monkeypatch) -> None:
    """Ensure environment safely manages missing keys."""
